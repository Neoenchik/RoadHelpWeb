 ...
 *
 * Поведение:
 *  - Если NEXT_PUBLIC_YANDEX_MAPS_API_KEY пустой — рендерится placeholder
 *    с подсказкой добавить ключ. Приложение остаётся рабочим.
...
export function MapBlock({
  ...
  // ── Placeholder, если нет ключа
  if (!API_KEY) {
    return (
      <div
        className={cn(
          'relative grid place-items-center overflow-hidden rounded-2xl bg-surface-sunken',
          className
        )}
      >
        <div className="absolute inset-0 opacity-30 [background:repeating-linear-gradient(45deg,transparent,transparent_12px,#94a3b8_12px,#94a3b8_13px)]" />
        <div className="relative flex flex-col items-center gap-2 p-6 text-center">
          <Icon name="MapPinned" size={32} className="text-primary-500" />
          <p className="text-caption text-ink-700">
            Карта появится после добавления<br />
            <code className="rounded bg-ink-100 px-1">NEXT_PUBLIC_YANDEX_MAPS_API_KEY</code> в&nbsp;.env
          </p>
        </div>
        {centerPin && <CenterPin />}
      </div>
