...
function Inner() {
  ...
  const pay = useMutation({
    mutationFn: async () => (await api.post(`/api/orders/${id}/pay`)).data,
    onSuccess: () => router.replace(`/app/orders/${id}/review`),
    onError: (e: any) => toast.error(e?.response?.data?.detail ?? e?.response?.data?.message ?? 'Ошибка оплаты'),
  })
  ...
  return (
    <main className="mx-auto flex min-h-svh max-w-screen-sm flex-col p-4">
      ...
      <div className="space-y-3 pb-safe">
        <Button size="xl" block onClick={() => pay.mutate()} loading={pay.isPending}>
