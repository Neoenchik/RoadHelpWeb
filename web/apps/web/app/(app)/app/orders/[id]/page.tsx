...
function Inner() {
  ...
  const select = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.info('Мастер получил заявку, ожидайте ответ…')
    },
  })
    ...
    <main className="relative min-h-svh">
      ...
      <section className="absolute inset-x-0 bottom-0 z-20 rounded-t-3xl bg-surface-base p-4 pb-safe shadow-sheet">
        ...
        {(order.status === 'PENDING' || order.status === 'MATCHED') && (
          order.status === 'MATCHED' && order.executor ? (
            <WaitingPanel executor={order.executor} />
          ) : (
            <PendingPanel
              executors={executors ?? []}
...
function WaitingPanel({ executor }: { executor: ExecutorMini }) {
  return (
    <div className="space-y-4 py-2">
      <div className="flex items-center gap-3">
        <Avatar name={`${executor.first_name} ${executor.last_name ?? ''}`} src={executor.avatar_url ?? undefined} size="lg" />
        <div className="flex-1">
          <div className="text-h3">{executor.first_name} {executor.last_name ?? ''}</div>
          <div className="text-body text-ink-500">Смотрит вашу заявку…</div>
        </div>
        <Spinner size="md" />
      </div>
      <p className="text-caption text-ink-500 text-center">
        Обычно мастер откликается в течение нескольких секунд
      </p>
    </div>
  )
}
