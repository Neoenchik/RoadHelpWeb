...
function Inner() {
  ...
  const { data: order } = useQuery<Order>({
    ...
    queryFn: async () => (await api.get(`/api/executor/orders/${id}`)).data,
  ...
  const start = useMutation({
    mutationFn: async () => (await api.post(`/api/executor/orders/${id}/start`)).data,
    onSuccess: () => qc.invalidateQueries({ queryKey: ['executor', 'order', id] }),
    onError: (e: any) => toast.error(e?.response?.data?.detail ?? e.message),
  })
  ...
  const cta = (() => {
    ...
    if (order.status === 'ARRIVED') {
      return { label: 'Начать работу', onClick: () => start.mutate(), loading: start.isPending }
    }
