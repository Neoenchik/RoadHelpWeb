...
function Inner() {
  ...
  const { data: incoming } = useQuery<Order | null>({
  ...
  const accept = useMutation({
    ...
    onSuccess: (data: Order) => {
      ...
      router.push(`/executor/orders/${data.id}`)
    },
...
function Verification({ status }: { status: ExecutorProfile['verification_status'] }) {
  const router = useRouter()
  return (
    <main className="mx-auto max-w-screen-sm p-4">
      <Card className="space-y-4">
        <Badge variant="warning">Верификация: {status}</Badge>
        <h1 className="text-h2">Дождитесь подтверждения профиля</h1>
        <p className="text-body text-ink-700">
          Мы проверяем документы (паспорт, права, СТС). Обычно занимает до 24 часов.
        </p>
        <Button variant="ghost" block onClick={() => router.push('/executor/verification')}>
          Заполнить недостающие документы
        </Button>
      </Card>
    </main>
  )
